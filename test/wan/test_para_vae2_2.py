...
from fastvae.models.wan.para_vae2_2 import DistDecoder3D, DistEncoder3d
from fastvae.models.wan.vae2_2 import Decoder3d, Encoder3d
...
def _dist_worker_encoder(
    ...
    with torch.no_grad():
        ...
        out = model(x, feat_cache=feat_cache, feat_idx=[0])
...
def _dist_decoder_worker(
    ...
    with torch.no_grad():
        ...
        out = model(z, feat_cache=feat_cache, feat_idx=[0])
...
def _run_multi_rank_encoder(payload: dict, world_size: int) -> None:
    ...
    try:
        ...
        for other in outputs[1:]:
            assert_close(other, actual, atol=1e-5, rtol=1e-5)
        ...
        ref_model.eval()
        with torch.no_grad():
            ref_cache = [None] * payload["feat_cache_size"]
            warmup_x = payload["x"].to(device)[..., :1, :, :]
            _ = ref_model(warmup_x, feat_cache=ref_cache, feat_idx=[0])
            expected = ref_model(
                payload["x"].to(device), feat_cache=ref_cache, feat_idx=[0]
            ).cpu()
...
def _run_multi_rank_decoder(payload: dict, world_size: int) -> None:
    ...
    try:
        ...
        with torch.no_grad():
            ...
            _ = ref_model(
                warmup_z, feat_cache=ref_cache, feat_idx=[0], first_chunk=True
            )
            expected = ref_model(
                payload["z"].to(device), feat_cache=ref_cache, feat_idx=[0]
            ).cpu()
...
def test_dist_encoder3d_vae22(shape, world_size):
...
@pytest.mark.parametrize("world_size", [1, 8])
def test_dist_decoder3d_vae22(shape, world_size):
    if not dist.is_available():
        pytest.skip("torch.distributed is not available")
